<?php
include("../inc/header.php");
?>


<div class="mapouter"><div class="gmap_canvas"><iframe width="600" height="450" id="gmap_canvas"
 src="https://maps.google.com/maps?q=2271%20Chambly%20Rd%2C%20Longueuil%2C%20Quebec%20J4J%203Z5&t=&z=15&ie=UTF8&iwloc=&output=embed"
frameborder="0" scrolling="no" marginheight="0" marginwidth="0"></iframe><a href="https://www.bitgeeks.net"></a></div>
<style>.mapouter{float:left;margin:30px;height:494px;width:600px;}.gmap_canvas
  {overflow:hidden;background:none!important;height:494px;width:600px;}</style></div>
    <br><br>
<div id="Info">
<div>

    <h2>Une équipe de confiance à votre service</h2>
    <hr>
        <p id="Heures"><b>HEURES D’OUVERTURE</b>
            <br>Du lundi au vendredi<br> de 8h30 à 17h00
            <br>Le Samedi de 9h00 à 1600<br>Fermé le Dimanche</p>

    <div class="icon fa-map-marker">
            2271 Chemin Chambly
            Longueuil, QC J4J 3Z5
            Canada
    </div><br>
    <a href="#" class="icon fa-phone"><span><b>TÉLÉPHONE</b><br>1(450) 647 2000</span></a><br>
    <a href="#" class="icon fa-envelope"><span><b>COURRIEL </b><br> garagecheminchambly@gmail.com</span></a><br>
    <b>RÉSEAUX SOCIAUX</b> <br>
    <a href="https://www.facebook.com/pg/Chemin-Chambly-Auto-Service-128131274628393/about/" class="icon fa-facebook"><span> Garage Chemin Chambly</span></a>
   <br> <a href="https://www.instagram.com/garagecheminchambly/" class="icon fa-instagram"><span> Garage Chemin Chambly</span></a>

</div>
</div>
<?php
include(PREAMBLE.'inc/scripts.php');
?>
</body>
</html>
